...
public class MiddleJunction extends CommandBase {
  /** Creates a new GrabPosition. */
  double x;
  boolean flag;
  public MiddleJunction() {
  ...
  public void execute() {
    ...
    if(!flag){
      if(RobotContainer.extend.get_extend_encoder() <= -3.0){
        ...
        RobotContainer.extend.extend(0.0);
        flag= true;
      if(RobotContainer.pivotOne.get_first_pivot_encoder() >= 23.0){
        RobotContainer.pivotOne.move_pivot_one(-0.2);
      }else if(RobotContainer.pivotOne.get_first_pivot_encoder() <= 19.0){
        RobotContainer.pivotOne.move_pivot_one(0.2);
      }else{
        RobotContainer.pivotOne.move_pivot_one(0.0);
      }

      if(RobotContainer.pivotTwo.get_second_pivot_encoder() >= -63.0){
        RobotContainer.pivotTwo.move_pivot_two(-0.3);
      }else if(RobotContainer.pivotTwo.get_second_pivot_encoder() <= -59.0){
        RobotContainer.pivotTwo.move_pivot_two(0.3);
      }else{
      ...
      // System.out.println(((RobotContainer.pivotOne.get_first_pivot_encoder() >= (2.0-5.0)&&RobotContainer.pivotOne.get_first_pivot_encoder() <= (2.0+5.0))));
       if((RobotContainer.pivotTwo.get_second_pivot_encoder() <= (-62.0+5.0)&&RobotContainer.pivotTwo.get_second_pivot_encoder() >= (-61.0-5.0)) && (RobotContainer.pivotOne.get_first_pivot_encoder() >= (21.0-5.0)&&RobotContainer.pivotOne.get_first_pivot_encoder() <= (21.0+5.0))){
         if(RobotContainer.extend.get_extend_encoder() >= -3){
           RobotContainer.extend.extend(-0.25);
         }else{
           RobotContainer.extend.extend(0.0);
           end(true);
          }
