...
public class HighJunctionAuto extends CommandBase {
  /** Creates a new GrabPosition. */
  double x;
  boolean flag;
  public HighJunctionAuto() {
  ...
  public void execute() {
    ...
    if(!flag){
      ...
      if(RobotContainer.pivotTwo.get_second_pivot_encoder() >= -88.0){
        RobotContainer.pivotTwo.move_pivot_two(-0.45);
      }else if(RobotContainer.pivotTwo.get_second_pivot_encoder() <= -92.0){
        RobotContainer.pivotTwo.move_pivot_two(0.45);
      }else{
      ...
      if((RobotContainer.pivotTwo.get_second_pivot_encoder() <= (-90.0+5.0)&&RobotContainer.pivotTwo.get_second_pivot_encoder() >= (-90.0-5.0)) && (RobotContainer.pivotOne.get_first_pivot_encoder() >= (44.0-5.0)&&RobotContainer.pivotOne.get_first_pivot_encoder() <= (44.0+5.0))){
        if(RobotContainer.extend.get_extend_encoder() >= -48){
