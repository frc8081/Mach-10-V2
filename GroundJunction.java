...
public class GroundJunction extends CommandBase {
  /** Creates a new GrabPosition. */
  double x;
  boolean flag;
  public GroundJunction() {
  ...
  public void execute() {
    ...
    if(!flag){
      ...
      if(RobotContainer.pivotTwo.get_second_pivot_encoder() >= -18.0){
        RobotContainer.pivotTwo.move_pivot_two(-0.3);
      }else if(RobotContainer.pivotTwo.get_second_pivot_encoder() <= -22.0){
        RobotContainer.pivotTwo.move_pivot_two(0.3);
      }else{
      ...
      if((RobotContainer.pivotTwo.get_second_pivot_encoder() <= (-20.0+5.0)&&RobotContainer.pivotTwo.get_second_pivot_encoder() >= (-20.0-5.0)) && (RobotContainer.pivotOne.get_first_pivot_encoder() >= (2.0-5.0)&&RobotContainer.pivotOne.get_first_pivot_encoder() <= (2.0+5.0))){
