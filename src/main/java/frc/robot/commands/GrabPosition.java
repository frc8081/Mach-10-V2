...
public class GrabPosition extends CommandBase {
  ...
  public void execute() {
    ...
    if(!flag){
      if(RobotContainer.extend.get_extend_encoder() <= -5.0){
      ...
      if((RobotContainer.pivotTwo.get_second_pivot_encoder() <= (-3.0+5.0)&&RobotContainer.pivotTwo.get_second_pivot_encoder() >= (-3.0-5.0)) && (RobotContainer.pivotOne.get_first_pivot_encoder() >= (2.0-5.0)&&RobotContainer.pivotOne.get_first_pivot_encoder() <= (2.0+5.0))){
        if(RobotContainer.extend.get_extend_encoder() >= -60){
          RobotContainer.extend.extend(-0.4);
        }else{
          RobotContainer.extend.extend(0.0);
          end(true);
